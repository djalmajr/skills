...
**multiple ai-memory instances** (e.g. a personal `memory.djalmajr.dev`, a separate org
instance), so never hardcode the endpoint:

1. **MCP endpoint** — the ai-memory instance URL (e.g. `https://memory.djalmajr.dev/mcp`) and
   the server name to register it under (e.g. `memory-personal`, `memory-zomme`). Ask which
   instance this repo should talk to.
2. **Workspace** — personal → `default`; org repo → its own workspace (e.g. `zommehq`).

...
# spawn must not treat an unqueryable idle match as reusable or as gone.
(
  set -euo pipefail
  ...
  printf '# name\tpane\tkind\trole\tfamily\tcreated_pane\tcwd\tstarted\n' > "$tsv"
  printf 'stuck\tp2\tgrok\timplementer\txai\t1\t/tmp/work\tnow\n' >> "$tsv"
  printf 'idle1\tp4\tgrok\timplementer\txai\t1\t/tmp/work\tnow\n' >> "$tsv"
  out="$(find_reusable implementer grok /tmp/work)" && rc=0 || rc=$?
  [ "$rc" -eq 0 ] || { printf 'FAIL: idle sibling not preferred (rc %s, out %s)\n' "$rc" "$out" >&2; exit 1; }
  [ "$out" = idle1 ] || { printf 'FAIL: reused %s\n' "$out" >&2; exit 1; }

  printf '# name\tpane\tkind\trole\tfamily\tcreated_pane\tcwd\tstarted\n' > "$tsv"
  printf 'stuck\tp2\tgrok\timplementer\txai\t1\t/tmp/work\tnow\n' >> "$tsv"
  out="$(find_reusable implementer grok /tmp/work)" && rc=0 || rc=$?
  [ "$rc" -eq 4 ] || { printf 'FAIL: unqueryable match rc %s out %s\n' "$rc" "$out" >&2; exit 1; }
  case "$out" in unavailable*$'\t'*PermissionDenied*$'\t'stuck) ;; *) printf 'FAIL: unqueryable out %s\n' "$out" >&2; exit 1 ;; esac

  printf '# name\tpane\tkind\trole\tfamily\tcreated_pane\tcwd\tstarted\n' > "$tsv"
  printf 'dead\tp3\tgrok\timplementer\txai\t1\t/tmp/work\tnow\n' >> "$tsv"
  out="$(find_reusable implementer grok /tmp/work)" && rc=0 || rc=$?
  [ "$rc" -eq 1 ] || { printf 'FAIL: real absence rc %s out %s\n' "$rc" "$out" >&2; exit 1; }
  [ -z "$out" ] || { printf 'FAIL: real absence produced %s\n' "$out" >&2; exit 1; }

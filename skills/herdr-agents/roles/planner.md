...
4. Propose the slices for the recommended approach: goal, owned files, forbidden files, order and dependencies, suggested role (`scouter`, `designer`, `implementer`, `tasker`), checks the worker may run. Slices must have disjoint files; shared resources are delivered ready in a brief or owned by exactly one slice.
